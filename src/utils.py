        ...
        if not args["training_method"]["run_active_learning"]:
            if not args["model"]["list_of_models"]:
                unique_results_identifier = (
                    f"{args['model']['model_name_or_path']}/non_active_one_model/{ts}"
                )
            else:
                unique_results_identifier = (
                    f"{args['model']['list_of_models'][0]}/non_active_majority/{ts}"
                )
        else:
            adapter_type = args["training_method"]["adapters"]["adapter_name"]
            if args["training_method"]["type"]:
                unique_results_identifier = (
                    f"{args['model']['model_name_or_path']}/active_pool_based/{adapter_type}/{ts}"
                )
            else:
